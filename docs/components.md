...
- columns 2–4: `flex flex-col gap-2.5`; title `<span id="footer-pages-title|footer-legal-title|footer-social-title" class="font-display text-base tracking-wider text-yellow uppercase">`; links inside `<nav [attr.aria-labelledby]>` › `<ul class="m-0 list-none p-0">`, each `text-sm text-white no-underline transition-colors duration-150 ease-in-out hover:text-yellow focus-ring`.
- column 4 icon row `flex gap-3`, each link `target="_blank" rel="noopener noreferrer"` with both `title` and `aria-label`, `p-1 -m-1` to grow the hit box to 28×28 without moving the 12px visual gap, glyph `<bfs-brand-icon [size]="20" />`.
- bottom bar `mx-auto mt-6 flex max-w-6xl flex-wrap justify-between gap-4 border-t border-neutral-600 px-6 py-4`, both spans `text-xs text-neutral-400`; the year is interpolated from a `currentYear` signal, never baked into the translation; `Deutsch`/`English` are real locale links following the identical switcher rule (`footer.language.de` / `footer.language.en`, endonyms in both locales; `footer.language-list` is retired).
