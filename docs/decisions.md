...
| Routing | Hydration | `provideClientHydration(withEventReplay())` — the burger must survive a tap during the hydration window. No `withIncrementalHydration()` (no deferred blocks). Criterion: a burger click fired before hydration still opens the panel. |
