...
| Repo readiness | README vs SPEC §5 command list | The README documents only commands that exist in `package.json` at that point, plus one sentence naming the SPEC §5 commands that arrive later. Criterion in both issues becomes the subset direction: every command named in the README exists as a script in `package.json`. |
...
| SEO service | `og:url` on noindex pages | Move `og:url` behind the `noIndex` guard, together with canonical and hreflang. A noindex page emits title, description, `robots: noindex, follow`, `twitter:card` and six `og:*` tags (`title`, `description`, `image`, `type`, `site_name`, `locale`). Criterion: every indexable page carries seven `og:*`; `/404/` carries six and no `og:url`, canonical or alternate. Mirror it in the 404 issue. |
...
| Legal layout | Text gate scope | `tools/verify-legal-text.mjs` (`npm run verify:legal`, landed with #30) compares the whitespace-normalised text of the `bfs-legal-page` prose wrapper against the `.final` source, per route and per locale. Both `<h1>`s are dropped before comparing (see _Furniture inside the wrapper_). A route whose page does not use the legal layout yet reports `SKIP` and is counted in the summary line — never a silent pass. |
...
| Legal layout | Furniture inside the wrapper | `tools/verify-legal-text.mjs` strips every element carrying `data-legal-not-in-source` before comparing. Exactly three elements have it: both H1s and the privacy "Stand" line — every element inside the wrapper that is translated. **Corrected while building #33:** the privacy H1 was initially left unmarked because `Datenschutzerklärung` is the supplied text's own first heading, which passes under `/de/` and fails under `/en/`, where the same element renders `Privacy Policy`. The source file's first `<h4>` is therefore dropped on the source side as well, so the comparison covers exactly the prose that is German in both locales. Marking in the template rather than matching on text is what makes the gate survive M7's English targets. |
...
| Legal pages in English | The four English targets | Authored while building #33, with Manu: `imprint.title` → **Legal notice** (not *Imprint*, which reads to native speakers as a publisher's imprint), `privacy.title` → **Privacy Policy**, `privacy.last-updated` → **Last updated: {INTERPOLATION}**, `legal.german-only-notice` → **This page is available in German only. Only the German version is legally binding.** These four are the whole English surface of both legal pages; everything else under `/en/` is the German prose by construction. **This supersedes the table in issue #38**, which proposes `IMPRINT` and `As of {date}` and shows the German sources in display caps — the sources are natural case since the brand-and-typography convention landed, and `Legal notice` was chosen over `Imprint` deliberately. Do not "restore" that table. |
...
| Translation sync | Where the workflow is written down | `docs/i18n-workflow.md`, linked from SPEC §6 and referenced by the two new entries in SPEC §5. It covers adding, changing and removing a string, what `i18n:check` asserts, the four ways it fails with the exact message each prints, and the two traps (one id in two templates whose German drifts apart — extraction only warns; ICU sub-message hash ids changing under an explicit `@@id`, which surfaces as an orphan rather than a missing unit). |
| Translation sync | The check itself | `"i18n:check": "ng extract-i18n && git diff --exit-code -- projects/website/src/locale && node tools/verify-translations.mjs"`, run as a step of the existing `build` job before the build. `verify-translations.mjs` asserts that no `<segment state="initial">` survives (the stale-after-source-change case) and that every unit has a non-empty `<target>`. Restated criteria: forgotten extraction → dirty tree; changed German source → `state="initial"`; deleted `<target>` or orphaned unit → red `verify:translations`. |
| Translation sync | `i18nMissingTranslation` does not cover a missing `<target>` | **Measured while building #34, and it invalidates the safety net this milestone was resting on.** `ng-extract-i18n-merge` writes every new unit into `messages.en.xlf` as `<segment state="initial">` with a `<source>` and no `<target>`. Angular resolves such a unit to its `<source>`, so `npx ng build --configuration en` with `i18nMissingTranslation: "error"` reports **zero** errors while 110 of 144 units render German under `/en/`. Deleting an existing `<target>` behaves the same way: build green, `Hauptnavigation` in the English HTML. `newTranslationTargetsBlank: "omit"` is still right — it is what keeps the German text out of the target — but it cannot make the build fail. `tools/verify-translations.mjs` is therefore the gate, not a second opinion, and it landed with #34 rather than #39 so the remaining M7 issues have something that actually checks them. Every M7 issue's `grep "No translation found"` step is superseded by `npm run verify:translations -- --prefix=<its ids>`. |
| Translation sync | What `verify:translations` asserts | Per unit in scope: present in `messages.en.xlf`, non-empty `<target>`, not `state="initial"`, `<source>` identical in both files, the `<ph id>` set identical between source and target, and no `→` in the target. Plus: no unit in `messages.en.xlf` that is absent from `messages.xlf`. `--prefix=a.,b.` narrows the scope so a milestone issue can prove its own ids while the rest of the catalogue is still untranslated; with no argument it checks all 144 and is what CI runs. |
...
| Issue | Question | Decision |
|---|---|---|
| Sitemap | Which pages a description change bumps | Refined while building #40: editing the **German** `seo.*.description` bumps only the German page's `lastmod`, not both members of the locale pair — the English page renders its own target and its fingerprint is unchanged. The issue's criterion ("bumps exactly that route's two pages") assumed one shared description. Measured: `dist/changed-urls.json` holds exactly `https://boundfoxstudios.com/support/`. Both members move only when both sides of the string change. |
...
| Sitemap | Alternates count | `const EXPECTED_ALTERNATES = 3;` in `tools/seo/sitemap.lib.mjs` with the comment that it mirrors `LOCALES.length + 1` and that `site.config.ts` is TypeScript and cannot be imported from an `.mjs` script. |
...
| Finalize dist | `ROOT_ONLY` list | `['robots.txt', 'favicon.ico', 'icon.svg', 'apple-touch-icon.png', 'icons', 'og', '.well-known']` — `og/` was missing and would ship a duplicate nothing references. Criterion: none of those seven entries survives under `dist/website/browser/en/`. The positive form this row first proposed cannot be asserted — Angular also duplicates the hashed bundles, `fonts/` and `images/` into every locale directory. |
...
| robots + JSON-LD | schema.org validation | A one-off validator.schema.org run recorded as a PR note. The machine checks this row first proposed died with `verify-dist` (SPEC §12 **D13**); the shape of every block is fixed in `json-ld.ts` and the catalog it maps over. |
...
| Icons + OG | "no colour outside the token set" | Restate as a check on chosen values, not rendered pixels: the generator's only colour constant is `const BACKGROUND = '#171717';`, and `#171717` is the only hex literal in the `index.html` head block and both `manifest.webmanifest` files. Checked by reading those three files, not by a script (SPEC §12 **D13**). |
| Build output assertions | Does the script exist | **No.** Decided by Manu while building M8 and recorded as SPEC §12 **D13**: `tools/verify-dist.mjs` is not written and issue #44 is closed unimplemented. Every `verify:dist` mention in `SPEC.md`, `README.md` and the rows above goes with it; the M8 gates hang directly off `npm run build`. |
...
| Gates | Jobs or steps | Two **steps** appended to the existing `build` job after `npm run build`, with `npx playwright install chromium` inserted before the axe step. **Without `--with-deps`**, corrected after it cost 12 minutes on a green run and hung until cancellation on the next: it runs `apt-get update` on the runner, and the `ubuntu-latest` image already carries the libraries Chromium needs. One `npm ci`, one build, one required check (`Build`). |
...
| bug-a-ball.com | Evidence and regression | Record issuer, SAN list, not-after date, auto-renew state and the redirect target in `docs/infrastructure.md`, and check the certificate by hand at the cutover — the weekly link-check job that was to watch it does not exist (SPEC §12 **D14**). |
...
| Link check | Is there one at all | **No.** Dropped while building #54 and recorded as SPEC §12 **D14**. Measured against the real build: `lycheeverse/lychee-action` v2.9.0 extracted 216 links from the 14 prerendered pages and every one of them came from the `<head>` — favicon, manifest, fonts, stylesheet, canonical, hreflang. No navigation link and no external destination was ever requested, with and without a `scheme` filter, so the job would have reported green while checking nothing. The nine-host coverage step that exposed it is exactly the kind of list that needs hand-updating whenever the copy gains a link, which is why it goes too. `.github/workflows/link-check.yml` and `lychee.toml` are deleted. |
...
| Post-launch | x-default | German (SPEC D9 beats the research sample): `X_DEFAULT_LOCALE = LOCALES[0]` in `site.config.ts`, the sitemap generator reads the same constant, and production is checked with `curl -s https://boundfoxstudios.com/en/support/ \| grep 'x-default'` → `href="https://boundfoxstudios.com/support/"`. |
