...
| Gates | Jobs or steps | Two **steps** appended to the existing `build` job after `npm run build`, with `npx playwright install --with-deps chromium` inserted before the axe step. One `npm ci`, one build, one required check (`Build`). |
