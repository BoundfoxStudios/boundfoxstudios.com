...
a:hover { color: #ffa726; }     /* --accent-strong */
```
...
```html
<section style="max-width:var(--container-max);margin:0 auto;padding:64px 24px 8px;">
```
...
```html
<section style="max-width:var(--container-max);margin:0 auto;padding:48px 24px 16px;">
```
...
- Body: `Maßstabsgetreue Plots für Lehrkräfte, passgenau auf 5×5-mm-Karopapier: Funktionen, Punkte, Flächen, Schrägbilder, Spiegelungen — und mit einem Schalter vom Arbeitsblatt zum Lösungsblatt.`
  *(`×` = U+00D7 multiplication sign, `—` = U+2014 em dash)*
...
```html
<section style="max-width:var(--container-max);margin:0 auto;padding:48px 24px 72px;">
```
...
```html
<image-slot id="bugaball-art-feature" shape="rect"
...
4. **Platform `<span>`**: `font-size:14px (--text-sm); color:#525252;`
   ...
   *(note: 14px here, whereas the small cards in §3.2 use 12px)*
5. **CTA `<a>`** — see §5.5.
...
<p i18n="@@apps-and-games.apps.mat.description">
  Markdown-Vorschau direkt aus dem Terminal — gerendert wie auf GitHub, inklusive Mermaid,
  KaTeX und Syntax-Highlighting. Kein Server, keine Konfiguration: so beiläufig wie
...
| `apps-and-games.games.bug-a-ball.key-art-alt` | *(to author)* | design has only the drop-slot placeholder `Bug-A-Ball Key-Art hier ablegen`, which is **not** shippable copy |
...
box-shadow: 0 10px 15px -3px rgb(255 193 7 / .3), 0 4px 6px -4px rgb(255 193 7 / .3);
...
*(The README states "Höhe 64px, Padding `0 24px`"; the file is the authority — `8px 24px` + `min-height:48px`. Same result, different mechanism; use the file's values so wrapped mobile rows grow correctly.)*
...
| **Bug-A-Ball key art** | *(none — `<image-slot>` placeholder)* | fills the media column, `object-fit: cover`, min-height 240px | — | **MISSING.** See below. |
