...
a    { color:var(--link); text-decoration:none; transition:color 150ms ease-in-out; }
...
**Critical for this page:** `active="legal"` matches *none* of the nav keys (`start`, `projekte`, `support`, `socials`). The header logic
...
| — | 40px | first H2 `margin-top` (= `space-7`/48 is *not* used; literal 40px) |
...
Usage in the file:
```html
<x-import component-from-global-scope="…Badge" variant="outline">Entwurf — vor Veröffentlichung rechtlich prüfen</x-import>
```
...
body { @apply bg-white font-sans text-neutral-700; }
a { @apply text-link no-underline transition-colors duration-150 ease-in-out hover:text-orange; }
```

Page template:

```html
<section class="mx-auto max-w-legal px-6 pt-16 pb-18">
  <h1 class="mb-2 font-display text-[clamp(40px,5.5vw,60px)] leading-none font-normal tracking-wide text-neutral-900">
    DATENSCHUTZERKLÄRUNG
  </h1>
  <app-badge variant="outline">Entwurf — vor Veröffentlichung rechtlich prüfen</app-badge>

  <h2 class="mt-10 mb-3 font-display text-2xl leading-none font-normal tracking-wide text-neutral-900">1. VERANTWORTLICHER</h2>
