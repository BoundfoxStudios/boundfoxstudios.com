...
```html
<h1
  style="margin:0;font-family:var(--font-display);font-weight:400;
...
```html
<h2
  style="margin:40px 0 12px;   /* first H2 only */
...
**Impressum passes `active="legal"`** ⇒ _no_ nav item is highlighted (all four render in the
...
- `#a16207` on white = 4.6:1 contrast (passes AA for body text); `#ffa726` hover on white is only
  ~2:1 — acceptable as a _hover_ accent alongside the persistent color difference, but do not use
