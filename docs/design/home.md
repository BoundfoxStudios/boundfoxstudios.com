...
4. **Gradient chip hover lift** — README mentions “Gradient-Pills heben sich 1px per translateY beim
   Hover”; the HTML has no transform, only a hover rule that *cancels* the link colour change.
...
| `home.projects.bug-a-ball.image-alt` | *(new — design has only the placeholder “Bug-A-Ball Key-Art hier ablegen”)* |
